...
s_ft=np.sqrt(sq**2/(m**2) + ((1/np.sqrt(2)-q)**2/(np.power(m, 4))*sm**2))
...
bx1.scatter(f_lin, phi_lin, label='phi=2pi*f*deltaT')
bx1.errorbar(f_lin, phi_lin, yerr=s_phi_lin, fmt='o')
bx1.axhline(0.5, color='green', linestyle='--', label='phi=0.5') 
bx1.set_ylabel('phi')
bx2.set_xlabel('f [Hz]')
bx1.legend()
bx1.plot(f_fit, phi_fit, label='fit lineare' )
bx1.grid(True)

bx2.errorbar(f_lin, residui1, yerr=s_phi_lin, fmt='o')
...
sm1, sq1 = err_param1
f_tlin1= (0.5 - q1)/m1
s_ft1=np.sqrt(sq1**2/(m1**2) + ((0.5-q1)**2/(np.power(m1, 4))*sm1**2))
